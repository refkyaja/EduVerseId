...
export default function ConfirmModal({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div
        onClick={!internalLoading ? onClose : undefined}
      ...
      <div className="relative z-10 w-full max-w-md bg-card border border-border/80 rounded-3xl p-6 sm:p-7 shadow-2xl animate-scale-in space-y-5 select-none">
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            ...
            className="w-8 h-8 rounded-full bg-muted/60 hover:bg-muted text-muted-foreground hover:text-foreground flex items-center justify-center transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex items-center gap-3 pt-2">
          <button
            ...
            disabled={internalLoading}
            onClick={onClose}
            className="flex-1 py-3 px-4 rounded-xl bg-muted hover:bg-muted/80 text-foreground font-extrabold text-xs transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {cancelText}
          </button>
          <Button
